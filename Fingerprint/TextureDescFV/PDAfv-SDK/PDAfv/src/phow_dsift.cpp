...
#define xy_step 3
#define magnif 6
#define windows_size 1.500000
#define contrastthreshold 0.0050000001
#define cSize 4
static int binSz[cSize] = {4, 6, 8, 10};


float* vl_dsift(float const* img, int rows, int cols, VlDsiftDescriptorGeometry geom, double bounds[], vl_bool useFlatWindow, 
			    bool norm, bool floatDescriptors, int verbose, int* feat_size, int* numKeypoint, double** frames_output)
{   
    /* Input */
    // I -> data, M, N
   ...
   * -------------------------------------------------------------- */
    ...
    //if (windowsSize >= 0) 
    vl_dsift_set_window_size(dsift, windows_size) ;
...
{	
	...
	for (int i = 0; i < cSize; i++)
	{
		...
		int num_keypoint = 0;
		double* frames = NULL;
		float* descr_scale = vl_dsift(img_vec_smooth, im_height, im_width, geom, bounds, VL_TRUE, true, true, false, &feat_size, &num_keypoint, &frames);		
