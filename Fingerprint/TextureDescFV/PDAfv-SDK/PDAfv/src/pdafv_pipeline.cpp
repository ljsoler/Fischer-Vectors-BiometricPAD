	...
	void training_fv_encoder_bmm(extract_params<unsigned char> params)
		{
			...
			long descriptor_size = 0;
			int db_size = imgs_url.size();
			unsigned char* samples_descriptors = NULL;
			//timeb ini, end;
			...
			{
				printf("Loading extracted features...\n");
				samples_descriptors = (unsigned char*)malloc(sizeof(unsigned char)*descriptor_size);
				LoadDescrBinary(string(params.outputDir) + descriptors_folder + sample_prefix + ext, samples_descriptors, descriptor_size);
			}
			else
			{
				...
				//random_shuffle(imgs_url.begin(), imgs_url.end());
				//GetLabels(imgs_url, "/live/");
				samples_descriptors = (unsigned char*)malloc(sizeof(unsigned char)*params.descriptors_count*params.features_dim);
				int numDescrsPerImage = params.descriptors_count/db_size;
				descriptor_size = params.descriptors_count*params.features_dim;
		#pragma omp parallel for
				for (int i = 0; i < db_size; i++)
				{
					...
					vector<unsigned char> desc = params.feature_extractor(img_i);
					//ftime(&end);
				...
				SaveDescrBinary(string(params.outputDir) + descriptors_folder + sample_prefix + ext, samples_descriptors, descriptor_size);
				string aux = string(params.outputDir) + descriptors_folder + "/samples_descriptors.csv";
				FILE* fl = fopen(aux.c_str(), "wb");
				for(int i = 0; i < descriptor_size/params.features_dim; i++)
				{
					for(int j = 0; j < params.features_dim; j++)
					{
						if(j != 0 && (j + 1) % (params.features_dim) == 0)
							fprintf(fl, "%i\n", samples_descriptors[i*params.features_dim + j]);
						else
							fprintf(fl, "%i,", samples_descriptors[i*params.features_dim + j]);
					}
				}
				fclose(fl);
			}
			...
			///		TRAINING GMM MODELS                ///
			//////////////////////////////////////////////
			float* means, *priors = NULL;
			bool mark = false;
			string output_path(params.outputDir);
			if (bmm::LoadBMMResultsBinary(output_path + "/", "pad", means, priors, params.num_cluster, dim) < 0)
			{
				printf("Training BMM model...\n");
				//Projecting DSIFT descriptor with PCA model
				int dimension = params.features_dim;
				int total_data = descriptor_size/params.features_dim;
				if(params.PCA_usage)
				...
				//Training GMM model
				BMM bmm_model = bmm::train_bmm_model(samples_descriptors, total_data, params.num_cluster, dimension, max_iter);
				printf("\tSave bmm model...\n");
				bmm::SaveBMMResultsBinary(params.outputDir, "pad", bmm_model);
				means = bmm_model.means;
				priors = bmm_model.priors;
			}
			free(samples_descriptors);
			descriptor_size = 0;
			////////////////////////////////////////////////////////////////////
			...
			if(!is_file_exist(t_path.c_str()) && !is_file_exist(tmp_fv.c_str()))
			{
				float* fv_descriptors = (float*)malloc(sizeof(float)*db_size*params.num_cluster*dim);
				int fv_dim = dim* params.num_cluster;
	#pragma omp parallel for
				for (int i = 0; i < db_size; i++)
				{
					printf("FV encoding image %d\n", i);
					...
					vector<unsigned char> desc = params.feature_extractor(img_i);
					unsigned char* feat_proj = &desc[0];
					long tmp_size = desc.size();
					if(params.PCA_usage)
					{
						//feature dimension reduction
					...
					float* enc = (float*)malloc(sizeof(float) * max_components * params.num_cluster);
					bmm::fisher_encode(enc, means, priors, feat_proj, tmp_size/max_components, max_components, params.num_cluster);
					if(params.PCA_usage)
						free(feat_proj);
					for (int j = 0; j < fv_dim; j++)
						fv_descriptors[i*fv_dim + j] = enc[j];
				...
				SaveDescrBinary(tmp_fv, fv_descriptors, db_size*params.num_cluster*dim);
				free(fv_descriptors);
			}
			if(params.PCA_usage)
			{
			...
			if(!is_file_exist(model_path.c_str()))
			{
				...
				for (int i = 0; i < nr_classes; i++)
				{
					...
					shuffle(index, db_size);
					double* labels = GetLabels(imgs_url, class_name);
					double* random_labels = labels;
					float* random_fv = fv_descriptors;
//					double* random_labels = (double*)malloc(sizeof(double)*db_size);
//					float* random_fv = (float*)malloc(descriptor_size*sizeof(float));
//
//					for (int j = 0; j < db_size; j++)
...
//						random_labels[j] = labels[index[j]];
//					}
					//printf("After Training svm model...\n");
					//Setting svm paramters
					VlSvmDataset* dataset = vl_svmdataset_new(VL_TYPE_FLOAT, random_fv, dim, db_size);
					double lambda = (double)1/(svm_C*db_size);
	...
	void testing_fv_encoder_bmm(extract_params<unsigned char> params)
	{
		...
		float* means = NULL, *priors = NULL;
		string output_path(params.outputDir);
		if (bmm::LoadBMMResultsBinary(output_path + "/", "pad", means, priors, params.num_cluster, params.pca_dim) < 0)
		{
			cout << "GMM MODEL NOT FOUND\n";
			return;
		}
	...
	#pragma omp parallel for
		for (int i = 0; i < db_size; i++)
		{
			...
			printf("Proccesing image %i from %i\n", i + 1, imgs_url.size());
			vector<unsigned char> desc = params.feature_extractor(img_i);
			unsigned char* reduced_descriptors = &desc[0];
			int dim_reduced_desc = desc.size();
			if(params.PCA_usage)
			{
				//reduced_descriptors = pca::project(&desc[0], desc.size()/ params.features_dim, params.features_dim, descriptors_means, eigenvectors, pca_dim);
			...
			float* enc = (float*)malloc(sizeof(float) * params.features_dim * params.num_cluster);
			bmm::fisher_encode(enc, means, priors, reduced_descriptors, desc.size()/params.features_dim, pca_dim, params.num_cluster);
			if(params.PCA_usage)
				free(reduced_descriptors);
			double probability[nr_classes];
			int model_size = dim;
